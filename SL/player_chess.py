...
class ChessPlayer:
    ...
    def get_top_moves(self, env, num_moves=3):
        self.reset()

        # 执行 MCTS 搜索
        root_value, naked_value = self.search_moves(env)
        policy = self.calc_policy(env)
        adjusted_policy = self.apply_temperature(policy, env.num_halfmoves)
        ...
        print(f"Total visits: {my_visit_stats.sum_n}")
        print(f"Non-zero children: {len([a for a, s in my_visit_stats.a.items() if s.n > 0])}")
        print(f"Top 5 policy values: {np.sort(adjusted_policy)[-5:][::-1]}")

        # 获取前 num_moves 个最佳走法
        # top_moves = []
        # for action, stats in my_visit_stats.a.items():
        #     move = action.uci()  # UCI 格式走法
        #     prob = adjusted_policy[self.move_lookup[action]]
        #     q_value = stats.q  # Q 值（-1 到 1）
        #     visits = stats.n  # 访问次数
        #     # 转换为胜率 (0 到 1)
        #     win_rate = (q_value + 1) / 2 if env.white_to_move else (1 - q_value) / 2
        #     top_moves.append((move, win_rate, visits))
        #
        # # 按访问次数排序（反映 MCTS 的信心）
        # top_moves.sort(key=lambda x: x[2], reverse=True)
        # return top_moves[:min(num_moves, len(top_moves))]
