...
def create_parser():
    ...
    parser.add_argument("--eval-type", help="type of evaluator to use", default="original",
                        choices=["original", "stockfish"])
    return parser
...
def start():
    ...
    if args.cmd == 'self':
        ...
        elif args.eval_type == "stockfish":
            from .worker import eval_stockfish  # Import the new evaluator
            return eval_stockfish.start(config)
    elif args.cmd == 'sl':
